...
class Details(Toplevel):
    def __init__(self, parent, vars):
        ...
        self.flanec = int(Create_kalitka.null_check(vars["Фланец"]))
        self.kosynka = int(Create_kalitka.null_check(vars["Косынка"]))
        self.sharnir = int(Create_kalitka.null_check(vars["Шарнир"]))
        self.zagl = int(Create_kalitka.null_check(vars["Заглушка"]))
...
class Density_set_class(Toplevel):
    ...
    def save_destr(self):
        ...
        data_prim = Create_kalitka.chgtocomas(var_new_dens)
...
class Create_kalitka(Tk):
    def __init__(self):
        ...
        self.height_kalitki = Entry(self, width=8)
        self.height_kalitki.grid(column=2, row=0)
        self.width_kalitki = Entry(self, width=8)
        self.width_kalitki.grid(column=3, row=0)
        ...
        self.sharnir_amount.insert(0, "2")
    ...
    def open_site_tube(self, event, w1, w2):
        ...
        bar = "&rt03[]="
        baz = foo + w1 + bar + w2
        #baz = foo + self.width1_1.get() + bar + self.width1_2.get()
        webbrowser.open(baz)
    ...
    def count(self):
        ...
        k_seb = self.read("kalitka_kefyy.json")["kefy"]["kalitka"]["seb"]
        k_sale = self.read("kalitka_kefyy.json")["kefy"]["kalitka"]["sale"]
    ...
    def to_fill(self):
        height = int(self.height_kalitki.get())
        ...
        if height > 2499:
            ...
            self.length1.delete(0, last=END)
            self.length1.insert(0, "{}".format(height1000))
        width = int(self.width_kalitki.get()) - 100
        width = width * 2 + height * 2
        self.dlina_ramy_1.delete(0, last=END)
        self.dlina_ramy_1.insert(0, "{}".format(width))
