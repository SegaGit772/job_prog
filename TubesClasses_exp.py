...
class Tubes(Toplevel):
    def __init__(self):
        ...
        """Создали окно ввода для получения кэфа для поиска в БД 1-прямоуг, круг, швеллера. 2 - поиск уголка"""
        self.db_search_kef = IntVar()
        self.db_search_kef.set(1)
        self.db_kef = Entry(self, textvariable=self.db_search_kef)
...
# решил работает-не трогай
    ...
    def database(self, width1, thickness, width2=0, k=1):
        try:
            sqlite_connection = sqlite3.connect('dens_db.db')
            cursor = sqlite_connection.cursor()
            sqlite_select_query = (f"""
                                        SELECT Density from test where width1={width1} 
                                        and width2={width2} and thickness={thickness} and kef={k}
                                    """)
    ...
    def tubecount(self):
        ...
        width1_1 = int(self.width1.get())
        width1_2 = int(self.width2.get())
        thickness1 = self.chgtocomas(self.thickness.get())
        tube_price1 = int(self.tubeprice.get())
        length1 = self.length1.get()
        kef1 = self.kef.get()

        tube_1_cost = int(self.database(width1_1, width1_2, thickness1, kef1) * tube_price1 * length1)
        """tube_2_cost    в будущем добавить еще одну трубу, заняться"""
        zagl_cost = 
